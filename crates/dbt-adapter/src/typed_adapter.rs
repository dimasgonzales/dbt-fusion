...
pub trait TypedBaseAdapter: fmt::Debug + Send + Sync + AdapterTyping {
    ...
    fn get_constraint_support(&self, ct: ConstraintType) -> ConstraintSupport {
        ...
        match (self.adapter_type(), ct) {
            ...
            // DuckDB
            // https://github.com/duckdb/dbt-duckdb/blob/481a80085a9e6d8eb10c908d6a40dd54a6798a02/dbt/adapters/duckdb/impl.py#L97
            (DuckDb, NotNull) => Enforced,
            (DuckDb, Unique) => Enforced,
            (DuckDb, PrimaryKey) => Enforced,
            (DuckDb, ForeignKey) => Enforced,
            (DuckDb, Check) => Enforced,
